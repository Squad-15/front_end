...
export default function ListaTrilhasComModulos({ searchTerm, categoria }: ListaTrilhasProps) {
  ...
  const trilhasFiltradas = trilhas.filter((trilha) => {
    ...
    const matchesSearch =
      normalize(trilha.name).includes(termo) ||
      normalize(trilha.description).includes(termo) ||
      normalize(trilha.category).includes(termo);

    const matchesCategoria = !cat || normalize(trilha.category) === cat;
